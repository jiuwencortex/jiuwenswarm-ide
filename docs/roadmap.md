...
| **Conversation export** | "↓ Export session" in the ☰ menu. Reads the session's `history.jsonl`, converts it to readable Markdown (User / Assistant headings, skips system messages), writes `jiuwenswarm-export-<title>.md` to the project root, and opens it immediately in the editor. |
...
| **Custom system prompt** | A free-text field in plugin settings that prepends user-defined instructions to every request — for example, preferred coding style, language, or persona. The project rules file (`.jiuwenswarm/instructions.md`) is project-scoped; this is a per-user, per-installation override that applies across all projects. No server changes needed. | Cursor (Custom instructions), Continue.dev (system message), GitHub Copilot (custom instructions) |
